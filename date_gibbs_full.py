import os, json, collections, pickle, sys
...
# ---- ordering constraints (root <= commentary) ----
REL=[]
...
for _cf in ['dcs_anchors.tsv','researched_anchors.tsv','chronbmm_priors.tsv','manual_constraints.tsv']:
...
# ---- constrained pairs: posterior ordering ----
print("\nROOT <= COMMENTARY posterior medians (constrained):",file=sys.stderr)
seen=set()
for w1,w2 in REL:
    if (w1,w2) in seen: continue
    seen.add((w1,w2)); i,j=widx[w1],widx[w2]
    tag='' if med[i]<=med[j]+2 else '  <<STILL VIOLATED'
    t1=meta.get(w1,{}).get('title','')[:26]; t2=meta.get(w2,{}).get('title','')[:26]
    print(f"   {t1:27s}{med[i]:6.0f}  <=  {t2:27s}{med[j]:6.0f}{tag}",file=sys.stderr)
